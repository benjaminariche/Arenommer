...
const INIT_EMPLOYEES = [
  {
    ...
    firstName: 'Marie',
    lastName: 'Lemaire',
    email: 'marie.lemaire@optiquedupont.fr',
  ...
  },
  {
    ...
    firstName: 'Julien',
    lastName: 'Renaud',
    email: 'julien.renaud@optiquedupont.fr',
  ...
  },
  {
    ...
    firstName: 'Sofia',
    lastName: 'Amrani',
    email: 'sofia.amrani@optiquedupont.fr',
...
const IMG = {
  ...
  disney:
    'https://images.unsplash.com/photo-1597466599360-3b9775841aec?w=400&h=250&fit=crop',
  parfum:
    'https://images.unsplash.com/photo-1541643600914-78b084683601?w=400&h=250&fit=crop',
  ...
  cinema:
    'https://images.unsplash.com/photo-1517604931442-7e0c8ed2963c?w=400&h=250&fit=crop',
};
...
const OFFERS = [
  {
    ...
    discount: '-47%',
    img: IMG.ugc,
    desc: 'Valable dans tous les cinémas UGC de France.',
    conditions: 'Valable 6 mois. Non remboursable.',
    stock: 'Illimité',
  },
  {
    id: 2,
    ...
    discount: '-30€',
    img: IMG.disney,
    desc: 'Accès 1 jour aux 2 parcs Disneyland Paris.',
    conditions: 'Billet daté, non modifiable.',
    stock: 'Selon dispo',
  },
  {
    id: 3,
    name: "Parfumerie de l'Europe",
    cat: 'Beauté',
    price: 0,
    display: '-35%',
    old: 'sur la sélection',
    discount: '-35%',
    img: IMG.parfum,
    desc: 'Grandes marques à tarifs exclusifs Perky.',
    conditions: 'Non cumulable.',
    stock: 'Selon dispo',
  },
  {
    id: 4,
    ...
    price: 47.5,
    display: '-5%',
    old: 'sur tout le site',
    discount: '-5%',
    img: IMG.fnac,
    desc: 'Utilisable en ligne et en magasin.',
    conditions: 'Valable 1 an.',
    stock: 'Illimité',
  },
  {
    id: 5,
    ...
    price: 0,
    display: '-20%',
    old: 'abonnement annuel',
    discount: '-20%',
    img: IMG.sport,
    desc: 'Tarif préférentiel Perky.',
  ...
  },
  {
    ...
    discount: '-25%',
    img: IMG.travel,
    desc: 'Résidences Pierre & Vacances et Center Parcs.',
    conditions: 'Selon dispo.',
    stock: 'Selon dispo',
  },
];
const CATEGORIES = [
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h2
            style={{ fontSize: 18, fontWeight: 500, color: t.text, margin: 0 }}
          >
            {title}
          </h2>
          <button
        ...
        {items.map((it) => (
          <button
            ...
            style={{
              ...
              borderRadius: 8,
              border: 'none',
              background: active === it.id ? t.blueLighter : 'transparent',
              color: active === it.id ? t.blue : t.textSec,
              cursor: 'pointer',
              fontSize: 13,
...
function BossScanner({
  ...
  const totalNavigo = Math.round(((s.navigo.pct - 50) / 100) * 86.4 * 12 * n);
  ...
  const globalPct = Math.round(
    ([totalPPV, totalNavigo, totalCadeaux, totalVacances].filter((v) => v > 0)
      .length /
      4) *
      100
  );
  ...
  const items = [
    {
      id: 'ppv',
      title: 'PPV 2026 — par salarié',
      icon: 'coin',
      color: 'blue',
      loi: 'Art. 1 loi n°2022-1158 — max 3 000€/salarié',
      total: totalPPV,
      pct: Math.round((totalPPV / (3000 * n)) * 100),
      perEmployee: true,
      savingsLabel: `${fmt(
        Math.round(totalPPV * 0.45)
      )}€ économisés vs primes classiques chargées`,
    },
    {
      id: 'navigo',
      title: 'Navigo — taux uniforme',
      icon: 'bus',
      color: 'amber',
      loi: "Art. L3261-4 CT — exo jusqu'à 75%",
      total: totalNavigo,
      pct: Math.round(((s.navigo.pct - 50) / 25) * 100),
      perEmployee: false,
      savingsLabel: `+${fmt(
        Math.round(((s.navigo.pct - 50) / 100) * 86.4 * 12)
      )}€/an net par salarié`,
    },
    {
      id: 'cadeaux',
      title: 'Chèques cadeaux Noël',
      icon: 'gift',
      color: 'amber',
      loi: 'Circ. URSSAF — plafond ~193€/événement',
      total: totalCadeaux,
      pct: Math.round((s.cadeaux.amount / 193) * 100),
      perEmployee: false,
      savingsLabel: `${fmt(
        Math.round(s.cadeaux.amount * n * 0.45)
      )}€ économisés vs primes cash`,
    },
    {
      id: 'vacances',
      ...
      total: totalVacances,
      pct: Math.round((s.vacances.amount / 550) * 100),
      perEmployee: false,
      savingsLabel: `${fmt(
        Math.round(s.vacances.amount * n * 0.22)
      )}€ de CSG/CRDS économisés`,
    },
    {
      id: 'resto',
      ...
      total: totalResto,
      pct: Math.round((s.resto.amount / s.resto.max) * 100),
      perEmployee: false,
      done: true,
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 14, color: t.textSec, margin: '0 0 20px' }}>
        Configurez chaque dispositif — le calcul se met à jour en temps réel
      </p>

      <div
        style={{
          background: `linear-gradient(135deg, ${t.blue} 0%, #0C447C 100%)`,
          borderRadius: 14,
          padding: '20px 24px',
          color: '#fff',
          marginBottom: 20,
      ...
      >
        <div>
          <div style={{ fontSize: 13, opacity: 0.8 }}>
            Pouvoir d'achat total configuré
          </div>
          <div style={{ fontSize: 36, fontWeight: 600 }}>
            {fmt(grandTotal)} €
          </div>
          <div style={{ fontSize: 13, opacity: 0.7 }}>
            pour {n} salarié{n > 1 ? 's' : ''} — ~
            {fmt(Math.round(grandTotal / Math.max(n, 1)))}€/pers.
          </div>
        </div>
        ...
        return (
          ...
          >
            ...
            >
              ...
              >
                <Icon name={item.icon} size={16} color={c} />
              </div>
              <div style={{ flex: 1 }}>
                <div
                  style={{
                ...
                >
                  ...
                  >
                    ...
                    <Badge
                      ...
                      variant={
                        item.done ? 'green' : item.total > 0 ? 'blue' : 'amber'
                      }
                      t={t}
                    />
                    <Icon
              ...
              >
                ...
                {item.perEmployee ? (
                  <div>
                    ...
                    >
                      Montant par salarié (modulable selon ancienneté ou
                      classification)
                    </div>
                    {activeEmp.map((emp) => (
                      <div
                      ...
                      >
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ fontSize: 11, color: t.textSec }}>
                            {emp.role} — {emp.seniority} mois d'ancienneté
                          </div>
                        </div>
                        <div
                          style={{
                    ...
                    >
                      <div>
                        <div style={{ fontSize: 11, color: c }}>
                          Total PPV équipe
                        </div>
                        <div
                          style={{ fontSize: 20, fontWeight: 600, color: c }}
                        >
                          {fmt(totalPPV)} €
                        </div>
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        ...
                        <div
                          style={{ fontSize: 16, fontWeight: 500, color: c }}
                        >
                          {fmt(3000 * n - totalPPV)}€ restants
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      >
        ⚠️ Estimations indicatives basées sur les plafonds légaux 2026. Valider
        avec votre expert-comptable avant mise en œuvre.
      </div>
    </div>
  );
}
  ...
  const addEmployee = () => {
    ...
    setEmployees((prev) => [
      ...prev,
      {
        ...
        id: Date.now(),
        initials,
        seniority: parseInt(form.seniority) || 0,
        ppv: 0,
        active: true,
      },
    ]);
    ...
    <div>
      ...
      >
        ...
        {active.length === 0 && (
          ...
          >
            Aucun salarié actif. Cliquez sur "Ajouter" pour commencer.
          </div>
        )}
        {active.map((emp, i) => (
          <div
            ...
            style={{
              ...
              alignItems: 'center',
              borderBottom:
                i < active.length - 1 ? `1px solid ${t.border}` : 'none',
            }}
          >
      ...
      >
        <span>
          Total : {active.length} salarié{active.length > 1 ? 's' : ''} × 5€ ={' '}
          <span style={{ color: t.text, fontWeight: 500 }}>
            {active.length * 5}€/mois
          </span>
        </span>
      ...
      {emp && (
        ...
        >
          ...
          <Input
            label="Prénom"
            value={emp.firstName}
            onChange={(v) => updateEmployee(emp.id, 'firstName', v)}
            t={t}
          />
          <Input
            label="Nom"
            value={emp.lastName}
            onChange={(v) => updateEmployee(emp.id, 'lastName', v)}
            t={t}
          />
          <Input
            label="Email"
          ...
          <Input
            label="Ancienneté (mois)"
            type="number"
            value={emp.seniority || 0}
            onChange={(v) => updateEmployee(emp.id, 'seniority', parseInt(v))}
            t={t}
          />
          <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
            <button
              onClick={() => setShowConfirmDelete(emp.id)}
              style={{
                flex: 1,
                padding: '10px',
                borderRadius: 8,
      ...
      >
        ...
        >
          Êtes-vous sûr de vouloir retirer ce salarié ? Son accès à Perky sera
          désactivé immédiatement et l'abonnement ajusté.
        </p>
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button
            onClick={() => removeEmployee(showConfirmDelete)}
            style={{
              flex: 1,
              padding: '10px',
              borderRadius: 8,
    ...
    <div>
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 14, color: t.textSec, margin: 0 }}>
          Optique Dupont — {n} bénéficiaire{n > 1 ? 's' : ''}
        </p>
      </div>
      <div
        style={{
      ...
      >
        ...
        <div
          style={{
            ...
            inset: 0,
            background:
              'linear-gradient(135deg, rgba(24,95,165,0.92) 0%, rgba(12,68,124,0.88) 100%)',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'space-between',
            padding: '0 32px',
        ...
        >
          <div style={{ color: '#fff' }}>
            <div style={{ fontSize: 13, opacity: 0.8 }}>
              Pouvoir d'achat récupérable
            </div>
            <div style={{ fontSize: 36, fontWeight: 600, margin: '4px 0' }}>
              {fmt(grandTotal)} €
            </div>
            <div style={{ fontSize: 13, opacity: 0.7 }}>
              soit ~{fmt(Math.round(grandTotal / Math.max(n, 1)))}€ par
              salarié/an
            </div>
          </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
        {[
          ...
          >
            ...
            >
              <Icon name={s2.icon} size={16} color={s2.c} />
              <span style={{ fontSize: 12, color: t.textSec }}>{s2.l}</span>
            </div>
      ...
      >
        ...
        >
          <Icon name="chart-bar" size={18} color={t.blue} /> Aperçu du scanner
        </div>
        <button
        ...
        {[
          {
            ...
            pct: Math.round((totalPPV / (3000 * n)) * 100),
            c: t.blue,
            badge: totalPPV > 0 ? 'blue' : 'amber',
            status: totalPPV > 0 ? 'Configuré' : 'À activer',
          },
          {
            title: 'Navigo',
            val: `${s.navigo.pct}%`,
            pct: Math.round(((s.navigo.pct - 50) / 25) * 100),
            c: t.amber,
            badge: s.navigo.pct > 50 ? 'blue' : 'amber',
            status: s.navigo.pct > 50 ? 'Optimisé' : 'Optimiser → 75%',
          },
          {
            title: 'Chèques cadeaux Noël',
            val: s.cadeaux.amount > 0 ? `${fmt(totalCadeaux)}€` : '—',
            pct: Math.round((s.cadeaux.amount / 193) * 100),
            c: t.amber,
            badge: s.cadeaux.amount > 0 ? 'blue' : 'amber',
            status: s.cadeaux.amount > 0 ? 'Configuré' : 'À activer',
          },
          {
            title: 'Titres-restaurant',
            val: `${s.resto.amount}€/billet`,
            pct: Math.round((s.resto.amount / s.resto.max) * 100),
            c: t.green,
            badge: 'green',
            status: 'Activé',
          ...
          >
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: it.c }}>
                  {it.val}
                </span>
...
function EmpHome({ employee, scannerState, t, onGoToCatalogue }) {
  ...
  const navigoExtra = Math.round(
    ((scannerState.navigo.pct - 50) / 100) * 86.4 * 12
  );
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 14, color: t.textSec, margin: '0 0 20px' }}>
        Bienvenue sur votre espace avantages Perky
      </p>
      ...
      {totalPerks > 0 && (
        ...
        >
          ...
          <div style={{ fontSize: 13, color: t.textSec, marginBottom: 16 }}>
            Ce que votre employeur vous apporte au-delà de votre salaire
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
            ...
            >
              ...
              <div style={{ fontSize: 20, fontWeight: 600, color: t.textSec }}>
                ●●●●●
              </div>
              <div style={{ fontSize: 11, color: t.textTert, marginTop: 4 }}>
                Connu de vous
              </div>
            </div>
            <div
            ...
            >
              <div style={{ fontSize: 11, color: t.green, marginBottom: 4 }}>
                Avantages en espèces
              </div>
              <div style={{ fontSize: 22, fontWeight: 600, color: t.green }}>
                {fmt(cashPerks)} €
              </div>
            ...
            >
              ...
              <div style={{ fontSize: 22, fontWeight: 600, color: t.blue }}>
                {fmt(naturePerks)} €
              </div>
              <div style={{ fontSize: 11, color: t.blue, marginTop: 4 }}>
                Transport + resto + vacances
              </div>
            </div>
          </div>
          ...
          <div
            style={{
              background: `linear-gradient(90deg, ${t.green}22, ${t.blue}22)`,
              borderRadius: 10,
              padding: '12px 16px',
              display: 'flex',
              justifyContent: 'space-between',
          ...
          >
            <div style={{ fontSize: 13, color: t.text }}>
              Avantages totaux offerts par votre employeur
            </div>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        >
          <Icon name="gift" size={18} color={t.blue} /> Ce que votre employeur
          vous offre
        </div>
        <div
          style={{
        ...
        >
          {[
            ppv > 0 && {
              title: 'PPV 2026',
              sub: `${fmt(ppv)}€ nets versés`,
              icon: 'coin',
              type: 'cash',
            },
            navigoExtra > 0 && {
              title: `Transport ${scannerState.navigo.pct}%`,
              sub: `${fmt(navigoExtra)}€/an supplémentaires`,
              icon: 'bus',
              type: 'nature',
            },
            scannerState.resto.amount > 0 && {
              title: 'Titres-restaurant',
              sub: `Pluxee — ${scannerState.resto.amount}€/jour`,
              icon: 'tools-kitchen-2',
              type: 'nature',
            },
            cadeau > 0 && {
              title: 'Chèques cadeaux',
              sub: `${fmt(cadeau)}€ à Noël`,
              icon: 'gift',
              type: 'cash',
            },
            vacances > 0 && {
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: 12,
                    padding: '16px 18px',
                    borderTop: `3px solid ${isCash ? t.green : t.blue}`,
                  }}
                >
                  <div
                    style={{
                  ...
                  >
                    <div
                      style={{
                        ...
                        height: 32,
                        borderRadius: 8,
                        background: isCash ? t.greenLight : t.blueLighter,
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'center',
                      }}
                    >
                      <Icon
                        name={it.icon}
                        size={16}
                        color={isCash ? t.green : t.blue}
                      />
                    </div>
                    <Badge
                      text={isCash ? 'Espèces' : 'Nature'}
                      variant={isCash ? 'green' : 'blue'}
                      t={t}
                    />
                  </div>
                  <div
          ...
          {OFFERS.slice(0, 4).map((o) => (
            ...
            >
              ...
              <div style={{ padding: '10px 12px' }}>
                ...
                <span style={{ fontSize: 15, fontWeight: 500, color: t.blue }}>
                  {o.display}
                </span>
              </div>
            </div>
...
function EmpCatalogue({
  ...
  const filtered = selectedCat
    ? OFFERS.filter((o) => o.cat === selectedCat)
    : OFFERS;
  return (
    <div>
      <h1
      ...
      <div style={{ position: 'relative', marginBottom: 20 }}>
        <input
          placeholder="Rechercher une enseigne, un parc, une marque..."
          style={{
            width: '100%',
        ...
        />
        <div style={{ position: 'absolute', left: 14, top: 13 }}>
          <Icon name="search" size={18} color={t.textTert} />
        </div>
      </div>
      <div
        style={{
      ...
      >
        {CATEGORIES.map((c, i) => (
          <div
            key={i}
            onClick={() =>
              setSelectedCat(selectedCat === c.label ? null : c.label)
            }
            style={{
              borderRadius: 12,
              overflow: 'hidden',
          ...
          >
            ...
            <div style={{ padding: '12px 14px 14px' }}>
              <div onClick={() => onOfferClick(o)}>
                <div
                  style={{ fontSize: 11, color: t.textSec, marginBottom: 2 }}
                >
                  {o.cat}
                </div>
                <div
                ...
                >
                  <span
                    style={{ fontSize: 18, fontWeight: 500, color: t.blue }}
                  >
                    {o.display}
                  </span>
                  <span
    ...
    return (
      <div style={{ textAlign: 'center', padding: '80px 0' }}>
        ...
        <div style={{ fontSize: 14, color: t.textSec }}>
          Parcourez le catalogue pour ajouter des offres
        </div>
      </div>
    );
  return (
      ...
      <div style={{ display: 'flex', gap: 24 }}>
          ...
          {cart.map((item, i) => (
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                <div style={{ fontSize: 15, fontWeight: 500, color: t.blue }}>
                  {item.display}
                </div>
                <button
                ...
                >
                  <Icon name="trash" size={14} color={t.textSec} />
                </button>
              </div>
            </div>
          ))}
        ...
        <div style={{ width: 260 }}>
          ...
          >
            ...
            >
              Billet envoyé immédiatement dans votre wallet
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div>
      ...
      {[
        {
          name: '2x Places UGC',
          date: "Valable jusqu'au 30/06/2026",
          img: IMG.ugc,
        },
        {
          name: 'Disneyland Paris — 28/05',
          date: 'Billet pour 2 personnes',
          img: IMG.disney,
        },
      ].map((it, i) => (
        <div
      ...
      {[
        { n: 'Place Pathé — 12/03/2026', p: '8,20€' },
        { n: 'Carte Fnac 50€ — 25/12/2025', p: '47,50€' },
      ].map((h, i) => (
        <div
          key={i}
          style={{
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 28 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          >
            <span style={{ fontSize: 28, fontWeight: 500, color: t.blue }}>
              {offer.display}
            </span>
            <span
              style={{
      ...
      >
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 12, color: t.textSec, marginTop: 2 }}>
              S'adapte aussi automatiquement à votre système
            </div>
          </div>
          <button
            onClick={() => setDark((d) => !d)}
...
export default function App() {
  ...
  const updateEmployeePPV = useCallback((id, val) => {
    setEmployees((prev) =>
      prev.map((e) => (e.id === id ? { ...e, ppv: val } : e))
    );
  }, []);
  ...
  const addToCart = useCallback((offer) => {
    setCart((prev) => {
      ...
      return exists
        ? prev.map((i) =>
            i.id === offer.id ? { ...i, qty: (i.qty || 1) + 1 } : i
          )
        : [...prev, { ...offer, qty: 1 }];
    });
    setEmpPage('cart');
  ...
  const empViews = {
    home: (
      <EmpHome
        ...
        t={t}
        onGoToCatalogue={() => setEmpPage('catalogue')}
      />
    ),
    catalogue: (
      <EmpCatalogue
  ...
  const bossViews = {
    ...
    scanner: (
      <BossScanner
        ...
        onUpdateEmployeePPV={updateEmployeePPV}
        t={t}
      />
    ),
  ...
  return (
    ...
    >
      ...
      >
        {[
          { id: 'employee', icon: 'user', label: 'Espace salarié' },
          { id: 'employer', icon: 'briefcase', label: 'Espace patron' },
        ].map((m) => (
          <button
            key={m.id}
            ...
            style={{
              padding: '8px 20px',
              borderRadius: 8,
              border: mode === m.id ? 'none' : `1px solid ${t.border}`,
              fontSize: 13,
              cursor: 'pointer',
              fontFamily: font,
              fontWeight: 500,
      ...
      <div style={{ display: 'flex' }}>
        {mode === 'employee' ? (
          <>
            <Sidebar
              items={empNav}
              active={empPage === 'detail' ? 'catalogue' : empPage}
              onSelect={(id) => {
                setEmpPage(id);
                setSelectedOffer(null);
              }}
              user={{
                initials: activeEmployee.initials,
          ...
          <>
            <Sidebar
              items={bossNav}
              active={bossPage}
              onSelect={setBossPage}
              user={{ initials: 'PD', name: 'M. Dupont', sub: 'Dirigeant' }}
              role="Espace dirigeant"
              t={t}
              cartCount={0}
            />
            <div
            ...
            >
              {bossViews[bossPage] || bossViews.home}
            </div>
          </>
        )}
      </div>
